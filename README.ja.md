...
- Formal Plan モード：実装前に計画を明示的に作成、修正、承認し、計画と実行チェックリストをセッション内に保存します。
...
macOS または Linux では、次のコマンドで最新リリースをインストールできます：

```sh
curl -fsSL https://github.com/Zts0hg/foxharness/releases/latest/download/install.sh | sh
```

インストーラーは OS とアーキテクチャを自動検出し、ダウンロードした
アーカイブを検証して、`sudo` を使わずに `~/.local/bin/fox` へ
インストールします。必要に応じて、このディレクトリをシェルの `PATH` に
追加します。
...
対応する環境変数は `FOX_VERSION`、`FOX_INSTALL_DIR`、
`FOX_NO_MODIFY_PATH` です。コマンドラインオプションは環境変数より優先されます。

### Release バイナリを手動でインストール
...
- `Enter`：メッセージを送信。
- `Shift+Tab`：次の送信に使用するモードを Default と Formal Plan の間で切り替え。
...
| `/clear` | 表示中の会話をクリア。 |
| `/new` | 新しいセッションを開始。 |
| `/plan` | Formal Plan モードに移行。`/plan off` で Default に戻る。 |
...
```bash
fox exec "コードの確認だけして、ファイルは変更しないで"
...
Formal Plan モードでは、最後に送信した計画を現在のセッションの `PLAN.md` に保存します。承認後の実行チェックリストは同じセッションの `TODO.md` に保存され、どちらもプロジェクトルートには書き込まれません。
