/* Package logsearch implements the AgentOps-owned read-only log evidence capability. */
package logsearch
...
/* Tool searches configured local service logs without permitting path escape. */
type Tool struct {
	logDir  string
	openLog func(string) (io.ReadCloser, error)
}

/* MaxLineBytes is the scanner ceiling retained by the AgentOps capability contract. */
const MaxLineBytes = 1 << 20

/* New creates a Tool rooted at logDir. */
func New(logDir string) *Tool {
	return &Tool{logDir: logDir}
}

/* Name returns the model-visible capability name. */
func (t *Tool) Name() string {
	return "log_search"
}

/* ParallelSafe reports that bounded read-only log searches may overlap. */
func (t *Tool) ParallelSafe() bool {
	return true
}

/* Definition returns the existing AgentOps log-search schema. */
...
/* AssessPermission classifies valid searches as bounded read-only observation. */
...
/* Execute reads matching lines in source order within the configured bounds. */
...
func (t *Tool) open(name string) (io.ReadCloser, error) {
	if t.openLog != nil {
