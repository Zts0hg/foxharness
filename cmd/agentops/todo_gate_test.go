...
/* TestAgentOpsTurnPolicyBlocksFinalAnswerOnUncheckedTODO verifies that an
 * AgentOps run cannot finish while its session TODO.md still has unchecked
 * checklist items and update_todo is registered. */
func TestAgentOpsTurnPolicyBlocksFinalAnswerOnUncheckedTODO(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "TODO.md"), []byte("# TODO\n\n- [ ] unfinished step\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	policy := newAgentOpsTurnPolicy(root)
...
/* TestAgentOpsTurnPolicyAllowsFinalAnswerAfterTODOUpdate verifies that a
 * settled checklist no longer blocks the final answer. */
func TestAgentOpsTurnPolicyAllowsFinalAnswerAfterTODOUpdate(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "TODO.md"), []byte("# TODO\n\n- [x] finished step\n"), 0o644); err != nil {
		t.Fatal(err)
	}
