...
在 macOS 或 Linux 上，可使用以下命令安裝最新版本：

```sh
curl -fsSL https://github.com/Zts0hg/foxharness/releases/latest/download/install.sh | sh
```

安裝腳本會自動偵測作業系統與架構、驗證下載的壓縮檔，並在不需要 `sudo`
的情況下將 `fox` 安裝至 `~/.local/bin/fox`。如有需要，它也會將該目錄加入
Shell 的 `PATH`。
...
對應的環境變數為 `FOX_VERSION`、`FOX_INSTALL_DIR` 和
`FOX_NO_MODIFY_PATH`。命令列參數的優先順序高於環境變數。
