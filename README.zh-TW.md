...
- 規劃模式：可自動產生並使用 `PLAN.md`、`TODO.md` 及 `MEMORY.md`。
...
舊的 `-provider` 參數不再接受。請使用 `-llm-provider` 切換 provider profile，
或用 `-protocol` 指定 OpenAI/Claude 相容協定。
...
如果不希望這些檔案被提交到程式碼儲存庫，可以將它們加入 `.gitignore` 中。
