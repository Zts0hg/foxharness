...
- 计划模式：可自动生成和使用 `PLAN.md`、`TODO.md` 及 `MEMORY.md`。
...
技能会自动提供给模型，模型在需要时通过 `skill` 工具调用相应技能。你也可以直接以斜杠命令的形式运行技能（例如 `/go-refactor`）。
...
如果不希望这些文件被提交到代码仓库，可以将它们添加到 `.gitignore` 中。
