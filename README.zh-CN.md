...
在 macOS 或 Linux 上，可使用以下命令安装最新版本：

```sh
curl -fsSL https://github.com/Zts0hg/foxharness/releases/latest/download/install.sh | sh
```

安装脚本会自动检测操作系统与架构、校验下载的压缩包，并在无需 `sudo` 的
情况下将 `fox` 安装到 `~/.local/bin/fox`。如有需要，它还会将该目录添加到
Shell 的 `PATH` 中。
...
对应的环境变量为 `FOX_VERSION`、`FOX_INSTALL_DIR` 和
`FOX_NO_MODIFY_PATH`。命令行参数的优先级高于环境变量。
