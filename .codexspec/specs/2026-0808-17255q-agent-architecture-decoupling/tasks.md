...
- [ ] **T104 (`D-AUT-002`)** TDD schema-versioned typed stage and `pending`/`running`/`verified` transitions, explicit known-state migration, fail-closed invalid or future state, and verify-first resume without pipeline bypass; update `DV-AUT-002` only after Green. Dependencies: T103. Covers: REQ-011, NFR-006, NFR-010.
