...
- [ ] **T110 (`D-AUT-008`)** TDD item-scoped post-run `Drain`/`Close` ownership, per-run and final extraction joins, deterministic next-run and next-item memory visibility, cancellation and fresh bounded lifecycle cleanup, warning-only extraction failures, and retained worktree on undrained work; update `DV-AUT-008` only after Green. Dependencies: T109. Covers: REQ-011, NFR-006, NFR-010.
- [ ] **T111 (`D-AUT-009`)** TDD the exact concurrency domain of omitted or case-sensitive `serial`, reject whitespace, case, numeric, parallel, and future values before preconditions, and prove every accepted configuration remains serial without warning fallback; update `DV-AUT-009` only after Green. Dependencies: T110. Covers: REQ-011, NFR-006, NFR-010.
- [ ] **T112 (`D-AUT-010`)** TDD one typed correlated core outcome for success, failure, cancellation, exhaustion, and start failure; preserve session, run, committed partial, cause, retry class, and lifecycle evidence; verify every started attempt, persist retry correlation, enforce fresh-runner and cancellation policy, and never present partial evidence as success; update `DV-AUT-010` only after Green and clear the T044 correction stop after the complete Autodev and repository gates. Dependencies: T111. Covers: REQ-011, NFR-006, NFR-010.
