...
- [ ] **T097 (`D-CHD-002`)** TDD one immutable child execution snapshot shared by provider invocation, model identity and context window, proactive and reactive compaction, telemetry, and persisted compact state, including one explicit observable unknown-model fallback; update `DV-CHD-002` only after Green. Dependencies: T043, DEC-045. Covers: REQ-012, NFR-006, NFR-010.
