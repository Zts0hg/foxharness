...
**Status**: Discovery
**Last Confirmed**: 2026-08-10 16:16 +0800
...
- **Correction Authority**: T043 proved all six items as pre-existing defects. Their confirmed correction semantics are authoritative in DEC-045 and MUST be implemented through six independently Green Red-Green-Refactor commits before the ChildRun baseline freezes.
...
- **Correction Authority**: T044 proved all ten items as pre-existing defects. Their confirmed correction semantics are authoritative in DEC-046 and MUST be implemented through ten independently Green Red-Green-Refactor commits before the AutodevPipeline baseline freezes. `DV-AUT-007` exactly-once reporting means one durable logical event identity with idempotent consumption; an external terminal stream cannot guarantee exactly-once physical delivery across a process crash.
...
### DEC-044: Proven BenchmarkEval defects are corrected in its control plane before baseline freeze

- **Status**: confirmed
- **Decision**: The executable T042 proofs classified `DV-BEN-001` through `DV-BEN-007` as pre-existing defects. They MUST be corrected before T043 and immutable fixture generation. Each correction MUST follow Red-Green-Refactor and land as an independently Green defect-focused commit. Benchmark-owned evaluation, fixture, process, provenance, reporting, and exit behavior MUST remain outside the core turn engine.
- **DV-BEN-001 correction**: `Case` MUST own a positive `timeout_seconds`, defaulting to 600 when omitted. One derived context and deadline MUST cover fixture materialization, harness construction, runtime execution, ordered validation, and reporting inputs for one repeat. A command validation retains its two-minute ceiling but MUST use the earlier of that ceiling and the remaining case deadline. Parent cancellation and case timeout MUST stop starting work, terminate active work, and synthesize correlated terminal validation records for every unstarted validation so result cardinality and ordering remain deterministic. Timeout and cancellation MUST remain distinguishable. Terminal cleanup MUST NOT reuse an expired case context; `DV-BEN-004` cleanup uses one fresh background-derived context with a strict 30-second deadline, and failure to finish within it remains typed infrastructure evidence.
- **DV-BEN-002 correction**: Every accepted repeat MUST produce one typed result status: `completed`, `failed`, `cancelled`, `timed_out`, or `infrastructure_failed`; runtime outcome, evaluation verdict, and infrastructure cause remain separate. `Success` is true only for `completed` runtime/evaluation with every validation passing. `cmd/bench` MUST return exit code `0` only when every requested repeat succeeds, `1` when execution completed with any failed/cancelled/timed-out result, and `2` for flag, case-load, setup that prevents an accepted repeat, or report-write infrastructure failure. Human and JSON output MUST agree with the exit decision; partial results MUST be written when a report destination remains available.
- **DV-BEN-003 correction**: The composition root MUST resolve one immutable typed `BenchmarkRuntimeSpec` from the same values used to build provider/model, turn budget, tool surface, prompt/memory, compaction, permission, observation, and interaction behavior. Machine-readable fidelity and human-visible differences MUST derive from that spec; independently maintained behavioral claim strings MUST NOT be authoritative. This typed current-runtime specification is a benchmark control-plane DTO and MUST NOT introduce the future general runtime profile implementation early.
- **DV-BEN-004 correction**: Fixture materialization MUST accept directories and regular files only, reject every source symlink and unsupported file type, preserve source immutability, and confine destination creation beneath its owned workspace through rooted or equivalently race-resistant operations. `file_contains` MUST accept only a non-empty relative path to a regular file whose opened target remains beneath the workspace; traversal, absolute paths, separators that escape, and symlink targets MUST fail closed. Successful repeat workspaces remain retained for inspection; every failed, cancelled, timed-out, panicked, or partially initialized repeat workspace MUST be removed using one fresh background-derived context with a strict 30-second deadline, with cleanup failure or timeout recorded as infrastructure evidence rather than hidden.
- **DV-BEN-005 correction**: `-repeat` MUST be strictly positive and overflow-safe. `max_turns` MUST be zero (default 12) or positive; negative values fail case loading. YAML decoding MUST reject unknown and duplicate fields. Each validation MUST use a known type and exactly its required non-empty fields: `command.command`, or `file_contains.path` plus non-empty `file_contains.contains`; irrelevant fields MUST be rejected. Case ID, fixture, and prompt remain required after trimming, relative fixture paths resolve from the case-file directory, and deterministic validation precedence MUST report structural input errors before filesystem or runtime work.
- **DV-BEN-006 correction**: Command validation MUST use a dedicated executor that creates an isolated process group or platform-equivalent job boundary, captures stdout and stderr independently with a hard one-MiB limit per stream, and reports bounded truncation/overflow evidence. Parent cancellation, case timeout, validation timeout, output overflow, and process exit MUST terminate and reap the complete process tree with bounded TERM-to-KILL escalation. Overflow is a failed validation, not silent success. Validations keep YAML order and one result per entry; after cancellation or timeout, remaining entries receive synthetic terminal records and are not executed.
- **DV-BEN-007 correction**: The JSON schema MUST carry an explicit schema version, one-based repeat index, run ID, case-definition SHA-256 identity, deterministic fixture-tree SHA-256 identity, runtime status and cause, provider protocol/model, effective case and command deadlines, and the resolved runtime-spec/fidelity snapshot. Identities MUST be computed from immutable normalized inputs before mutable execution. Tests MAY normalize duration, workspace, session, run, and timestamps, but production reports MUST retain actual values. Corrected schema compatibility is required from its introduction forward; compatibility with the defective pre-baseline schema is not required.
- **Cross-Validation**: Local Codex separates typed timeout from cancellation, kills child process groups, caps retained stdout/stderr, bounds pipe drain, records typed completed/interrupted/failed states, and confines filesystem operations through rooted or canonical-path checks. Local Claude Code links work to `AbortSignal`, escalates process termination from TERM to KILL with tree cleanup, bounds shell output, checks original and resolved symlink paths, and records completed/failed/interrupted session states plus session/model metadata. Neither project owns a benchmark evaluator equivalent to Fox, so they support the lifecycle, process, output, path, terminal-state, and provenance principles but do not justify moving Fox fixture, fidelity, repeat, report, or exit-code policy into the core runtime.
- **Alternatives Rejected**: Per-stage unrelated timeouts without a case lifetime; treating timeout as ordinary validation failure; success exit after failed results; handwritten fidelity claims; following fixture symlinks; retaining failed partial workspaces; accepting vacuous validation fields; unbounded `CombinedOutput`; killing only the immediate shell; silent output truncation; unstable case IDs without content identity; and introducing the future general runtime profile architecture during this pre-refactor defect correction.
- **Reason**: These corrections create one bounded evaluation lifetime, explicit terminal and process semantics, one filesystem authority, deterministic input validity, bounded process resources, and reproducible result identity before the BenchmarkEval behavior baseline freezes.
- **User Evidence**: The user requested an additional source-level Codex and Claude Code cross-validation and authorized adoption when no conflicting design defect was found. No conflict was found; the comparison kept benchmark-specific policy in the BenchmarkEval control plane and strengthened process-tree, output-bound, rooted-path, and typed terminal requirements.

### DEC-045: Proven ChildRun defects are corrected through one frozen child contract before baseline freeze

- **Status**: confirmed
- **Decision**: The executable T043 proofs classified `DV-CHD-001` through `DV-CHD-006` as pre-existing defects. They MUST be corrected before T044 and immutable fixture generation. Each correction MUST follow Red-Green-Refactor, retain its trace evidence, and land as an independently Green defect-focused commit rather than as architecture refactoring. The corrected behavior MUST preserve synchronous one-level ChildRun execution and MUST NOT add background-child, resume, send-input, worktree, agent-tree, MCP, or team capabilities.
- **DV-CHD-001 correction**: A read-only child MUST retain the compatible `bash` tool name, schema, and model-visible presence, but execution MUST be incapable of persistent mutation. A conservative complete shell parse MUST reject unclassified commands, write redirection, background or detach constructs, unsafe environment assignment, dynamic execution, and write-capable command or flag forms before permission review or execution. An execution-layer sandbox MUST restrict filesystem access to declared readable roots, disable network mutation, and isolate the process environment so the child cannot modify the workspace, repository, outside paths, parent runtime environment, or persistent external state. The parser is defense in depth rather than the sole boundary. When the platform cannot establish the required sandbox, the Bash call MUST fail closed rather than run unrestricted. Approval or a parent grant MUST NOT expand this ceiling.
- **DV-CHD-002 correction**: Child admission MUST freeze one immutable execution snapshot containing provider protocol and instance, model identity, effective context window, model overrides, and compaction inputs. Model invocation, proactive and reactive compaction, token thresholds, telemetry, and persisted compact state MUST use that same snapshot. A compactor MUST NOT receive an empty model and independently fall back to 128K. An unknown model MAY use one explicit resolved fallback only when that fallback is frozen once and reported consistently to every consumer and observer. Parent model or configuration changes after child start MUST NOT affect the active child.
- **DV-CHD-003 correction**: Runtime MUST build one immutable effective child tool snapshot from the intersection of the profile ceiling, read-only or writable mode, selected-agent narrowing, and caller allowlist. Model-visible definitions, runtime execution, aliases, permission assessment, parallel-safety lookup, and prompt capability fragments MUST derive from that snapshot. Prompt text MUST describe write, edit, TODO, skill, interaction, or delegation capabilities only when present; the current ChildRun prompt therefore MUST NOT promise TODO, model-invocable skill, interaction, or nested delegation. A filtered capability MUST remain unreachable through a stale registry and MUST produce the stable unavailable-tool behavior.
- **DV-CHD-004 correction**: ChildRun Bash is synchronous and MUST reject shell-level background or detach behavior because ChildRun exposes no later handle or lifecycle protocol. Every spawned process MUST be registered before execution with a run-owned process supervisor and contained by a process group, job, or stronger platform-equivalent boundary. Success, provider or tool failure, turn exhaustion, cancellation, panic, and ordinary return MUST all execute one terminal cleanup that cancels pending permission work, terminates and reaps the complete child process tree with bounded escalation, and prevents late output, events, permission responses, reports, or side effects. Cleanup MUST complete before the unique parent-visible terminal outcome is published; cleanup failure MUST be represented as a correlated terminal failure rather than success.
- **DV-CHD-005 correction**: The fork `agent` value MUST be resolved before child-session creation and MUST NOT remain advisory. Empty and `general-purpose` select the current general-purpose ChildRun agent; an unknown non-empty value MUST fail explicitly instead of silently falling back. The normalized agent identity MUST enter the typed run specification, role or prompt fragment, lineage, outcome, and telemetry. In this correction, an agent MAY alter its persona and further narrow tools, but MUST NOT replace or expand the frozen provider, model, workspace, depth, permission, or turn ceiling. Agent-specific model selection remains outside scope and requires a separately confirmed requirement.
- **DV-CHD-006 correction**: Every accepted child invocation MUST produce one typed correlated terminal outcome distinguishing at least success, failure, cancellation, turn exhaustion, and rejection or start failure. The outcome MUST retain invocation and parent lineage and, after creation, child session and run identity. A permitted partial report is only the latest complete assistant text successfully committed to the authoritative child conversation; streaming fragments, pure tool calls, tool output, uncommitted responses, and stale reports are forbidden. Provider, tool, persistence, compaction, turn-limit, and cancellation failures MUST preserve their error classification while returning any permitted correlated outcome. `delegate_task` and fork adapters MUST retain both terminal error and partial outcome without presenting partial content as success. Terminal cleanup MUST precede outcome return, and one invocation MUST NOT publish more than one terminal outcome.
- **Cross-Validation**: Local Codex constructs child configuration from the parent's effective live turn, refreshes model, provider, permission, sandbox, and cwd, records parent/depth/role lineage, derives compaction thresholds from the same turn model, enforces read-only filesystem access below prompt semantics, drains tracked processes, and publishes typed completed or aborted turn events. Local Claude Code resolves `subagent_type` to a concrete agent definition, derives model, effective tools, and prompt from that definition, carries the resolved model through the child query and compaction path, removes recursive delegation, extracts the latest text-bearing assistant result on interruption, and performs agent-scoped cleanup in `finally`. Claude Code's command classifier is not sufficient as Fox's sole read-only boundary, and reference support for agent-selected models, background children, worktrees, MCP, teams, or deeper trees does not override Fox's confirmed ceilings.
- **Alternatives Rejected**: Prompt-only read-only guidance; permission approval as a sandbox substitute; a Bash classifier without execution containment; unrestricted fallback when sandbox setup fails; independently inferred compactor models; hard-coded prompt capability claims; cleanup only on cancellation; allowing detached child work without a handle; ignoring or silently defaulting unknown agent selectors; allowing agent selection to change parent-frozen runtime ceilings; returning nil for every failure after child identity exists; and reporting partial text as successful completion.
- **Reason**: These corrections create one immutable child execution contract spanning model, compaction, capabilities, prompt, process lifetime, agent selection, lineage, and terminal outcome. They establish the safe post-fix ChildRun behavior that characterization and later runtime extraction must preserve without prematurely implementing the target runtime architecture.
- **User Evidence**: After T043 proved all six defects, the user requested an additional source-level Codex and Claude Code cross-validation and gap audit, reviewed the resulting six correction semantics, and explicitly confirmed them.

### DEC-046: Proven Autodev defects are corrected through a durable deterministic control-plane contract before baseline freeze

- **Status**: confirmed
- **Decision**: The executable T044 proofs classified `DV-AUT-001` through `DV-AUT-010` as pre-existing defects. They MUST be corrected before T045 and immutable fixture generation. Each correction MUST follow Red-Green-Refactor, retain its trace evidence, and land as an independently Green defect-focused commit rather than architecture refactoring. The corrected contract MUST preserve serial Autodev orchestration, fixed CodexSpec stages, Engineer supervision, local worktree isolation, and core-runtime control-client behavior without introducing parallel items, cloud execution, background agents, a generic event bus, or the target runtime architecture early.
- **DV-AUT-001 correction**: Ledger mutation MUST be transactional. A mutation is applied to a candidate snapshot, atomically written, flushed, and committed before replacing authoritative in-memory state. Every stage or publication attempt MUST persist a stable intent or attempt identity before work begins, and every verified irreversible result MUST persist before the next stage, remote side effect, done report, or cleanup begins. Any save or durability failure MUST return a typed control-plane failure, stop subsequent core, Git, GitHub, done, and cleanup work, and retain the worktree. Restart MUST reconcile a persisted intent against deterministic stage ground truth and stable remote identity before retrying; it MUST NOT blindly repeat or silently continue after an unavailable ledger.
- **DV-AUT-002 correction**: Persisted pipeline state MUST use a schema-versioned typed stage and explicit `pending`, `running`, or `verified` transition state. Empty legacy state means the first stage only; known legacy stage values receive an explicit compatibility migration. Unknown, malformed, renamed without a declared migration, future-version, terminally inconsistent, or otherwise illegal state MUST fail as `InvalidLedgerState` before worktree creation, core execution, or remote access. A resumed `running` stage MUST perform read-only ground-truth verification first and persist `verified` when already satisfied; only an unsatisfied known stage may execute again.
- **DV-AUT-003 correction**: Every normalized backlog item MUST have an immutable durable `ItemID`; backlog Markdown MUST support an optional explicit `**ID**` source identifier, and ledger records MUST preserve `ItemID`, source identity, and the frozen requirement revision. Legacy records MAY bind once by a unique title, but ambiguous rename, deletion plus addition, duplicate-title movement, duplicate ID, or other uncertain correspondence MUST return an actionable reconciliation conflict instead of guessing. A pending item missing from the current backlog becomes non-runnable orphaned state; a missing in-progress item is blocked; a done item remains history. Current backlog order and priority govern pending scheduling, while advisory backlog status never overwrites durable progress. Transition to in-progress freezes the full title, description, byte length, and content hash used by that attempt; a later source edit requires explicit reconciliation rather than silently changing active work.
- **DV-AUT-004 correction**: Backlog parsing MUST preserve accepted UTF-8 requirement text without line-based truncation. The accepted document domain has a hard 64-MiB whole-file ceiling rather than a per-line Scanner ceiling; an oversize or invalid document fails atomically before work. Apart from documented field-separator and line-ending normalization, multiline Markdown, blank lines, fenced code, Unicode, and long descriptions MUST remain complete. Empty descriptions retain the existing deterministic title fallback. Materialized `requirements.md` MUST contain the complete authoritative text plus `ItemID`, byte length, and content hash; authoritative data MUST NOT contain an ellipsis or 4,000-character truncation. Presentation-only previews MAY be bounded independently.
- **DV-AUT-005 correction**: One rooted feature-workspace authority MUST validate and perform every Autodev artifact read and write. `FeatureDir` MUST be a normalized `.codexspec/specs/<feature-name>` relative path and MUST reject absolute paths, traversal, empty or malformed components, and illegal persisted values. Materialization, stage verification, review parsing, task inspection, and resume MUST operate through `os.Root` or an equivalently race-resistant rooted API. Every feature-path component and authoritative artifact target MUST be checked with no-follow metadata; feature directories and artifact files MUST reject symlinks, and final artifact targets MUST be regular files. Any violation fails before core or network work and MUST NOT be auto-reanchored or rewritten.
- **DV-AUT-006 correction**: Git and arbitrary command execution MUST return a structured result with stdout and stderr retained independently up to one MiB per stream plus explicit overflow evidence. A control-plane JSON query that overflows is a failure and MUST NOT parse truncated JSON; an ordinary quality gate MAY retain its exit-code result but MUST report truncation explicitly. Default ceilings are 30 seconds for read-only Git or GitHub queries, 10 minutes for a quality-gate command, and 30 minutes for one stage or core attempt; an earlier caller deadline wins, and no total retry-count abandonment budget is added. Every process MUST be registered in a process-group or platform-equivalent tree boundary before execution. Cancellation, timeout, or required termination MUST use bounded TERM-to-KILL escalation, wait for complete reaping under a fresh cleanup context, and suppress every later gate, stage, or remote operation. The CLI MUST propagate signals and map SIGINT to 130 and SIGTERM to 143.
- **DV-AUT-007 correction**: Every GitHub issue created for an item MUST carry an exact durable marker such as `<!-- fox-autodev-item-id:<ItemID> -->`. A recorded issue number MUST still be read and verified against repository and marker; title is mutable presentation data, and a matching closed issue remains reusable. Without a recorded number, verification MUST query every relevant page by marker: zero matches permits creation, one match binds it, and multiple matches fail as a conflict. The binding MUST persist successfully before PR work or reporting. Issue observation MUST use a durable logical `EventID` and outbox-style at-least-once delivery; adapters MUST consume that identity idempotently and MUST NOT emit duplicate logical events in one process. This is the achievable exactly-once logical reporting contract; physical stdout delivery cannot be guaranteed exactly once across a crash.
- **DV-AUT-008 correction**: The item-scoped `CoreRunner` contract MUST expose owned post-run lifecycle completion through `Drain` and `Close`, or an equivalent awaitable post-run handle. Automemory extraction remains asynchronous and non-terminal to the core result, and provider failure or panic remains isolated warning evidence, but Autodev MUST await extraction after each core run before another core run and MUST perform a final bounded drain before starting the next item, removing the worktree, returning, or exiting the process. Parent cancellation MUST cancel outstanding extraction and use a fresh bounded lifecycle context to join it. If post-run work cannot be cancelled and joined, cleanup MUST fail explicitly and retain the worktree; no item may observe memory whose availability depends on scheduler timing.
- **DV-AUT-009 correction**: The accepted `concurrency` domain is omission or the exact case-sensitive scalar `serial`. Leading or trailing whitespace, case variants, `parallel`, numeric values, future values, and other strings MUST fail during configuration loading before preconditions or work. No warning fallback is permitted, and every accepted configuration remains serial. A future parallel mode requires a separately confirmed enum value, real scheduler, isolation semantics, and characterization in the same behavior change.
- **DV-AUT-010 correction**: `CoreRunner.Run` MUST return one unambiguous typed `CoreOutcome` rather than an independently nullable result and error pair. The outcome MUST distinguish at least succeeded, failed, cancelled, turn-exhausted, and start-failed; preserve original cause classification and unwrap behavior; and retain session ID, run ID, latest permitted committed assistant partial result, side-effect correlation, retry class, and post-run lifecycle handle once established. Every started run, including failure and cancellation, MUST receive read-only stage verification; cancellation reconciliation uses a fresh bounded context but cannot authorize new work. A verified stage is durably completed, while cancellation still stops. An unverified retryable provider, tool, or turn-limit outcome may reach Engineer review with explicitly labelled partial evidence; session-persistence or corrupted-state outcomes require a fresh runner before recovery, and cancellation is never retried. Each retry MUST first persist attempt and correlation evidence. Partial content MUST never be presented as a successful final answer, and one attempt MUST produce exactly one terminal control-plane outcome.
- **Cross-Validation**: Local Codex uses fail-closed typed persisted status, stable job, item, source, thread, turn, and idempotency identities, conditional state transitions, retryable buffered persistence with explicit flush errors, bounded command output and deadlines, process registration and termination, and typed completed, failed, and interrupted turn outcomes. Local Claude Code retains full agent prompts under stable agent IDs, links child cancellation, registers asynchronous cleanup, bounds or spills command output, terminates process trees, checks original and resolved filesystem paths, validates configuration through strict schemas, and drains in-flight session and worktree cleanup during shutdown. Codex rollout history is not equivalent to Fox's authoritative side-effect ledger, and Claude's interactive background-command policy is not appropriate for unattended Autodev, so Fox intentionally uses stricter fail-closed persistence and no background process semantics.
- **Alternatives Rejected**: Warning and continuing after authoritative save failure; treating unknown state as post-pipeline; title, position, fuzzy text, or slug-only identity guesses; truncating authoritative requirements; check-then-open path validation; unbounded `CombinedOutput`; killing only an immediate shell; first-title or first-page GitHub matching; claiming impossible cross-crash exactly-once stdout delivery; unowned fire-and-forget extraction; accepting advertised but unsupported concurrency; discarding partial correlated outcomes; presenting partial evidence as success; and implementing the target runtime architecture as part of these pre-baseline corrections.
- **Reason**: These corrections establish one durable, typed, bounded, rooted, identity-stable, and lifecycle-owned Autodev control plane whose post-fix behavior can be characterized and then preserved through runtime and package refactoring.
- **User Evidence**: After T044 proved all ten defects, the user requested an additional source-level Codex and Claude Code cross-validation, reviewed the ten proposed correction semantics and the exactly-once reporting limitation, and explicitly confirmed all of them.
...
### Session 2026-08-10 14:14 +0800

- **Summary Presented**: T042 proved all seven BenchmarkEval risks. Local Codex and Claude Code source cross-validation supported one bounded cancellation-aware lifetime, typed terminal states, process-tree termination and reaping, bounded stdout/stderr, resolved/rooted filesystem authority, and explicit session/model provenance. Neither reference owns a Fox-equivalent evaluator, so fixture identity, repeat identity, runtime fidelity, report schema, and process exit policy remain benchmark control-plane responsibilities rather than core runtime behavior.
- **User Confirmation**: The user requested the additional source-level cross-validation and authorized adoption if no conflicting design defect was found. No conflict was found after fixing explicit semantics for the 600-second case default, a separate fresh 30-second cleanup context, exit codes 0/1/2, success-only workspace retention, one-MiB per-stream bounds, one-based repeat identity, and corrected pre-baseline schema compatibility.
- **Entries Confirmed**: DEC-044
- **Entries Amended**: CON-005

### Session 2026-08-10 16:16 +0800

- **Summary Presented**: T043 proved all six ChildRun risks. Local Codex and Claude Code source cross-validation supported execution-enforced read-only capability, one frozen provider/model/compaction snapshot, one effective tool snapshot shared with prompt rendering, run-scoped process supervision and terminal cleanup, behaviorally meaningful agent selection, and typed correlated partial outcomes. The comparison also established that Claude Code's command classifier cannot be Fox's sole read-only boundary and that reference-only model-selecting agents, background execution, worktrees, MCP, teams, or deeper agent trees do not expand Fox's confirmed one-level synchronous ChildRun profile.
- **User Confirmation**: The user explicitly confirmed all six proposed correction semantics after the additional source-level cross-validation and gap audit.
- **Entries Confirmed**: DEC-045
- **Entries Amended**: CON-005

### Session 2026-08-11 11:40 +0800

- **Summary Presented**: T044 proved all ten Autodev risks. Local Codex and Claude Code source cross-validation supported transactional fail-closed authoritative persistence, typed versioned stage state, stable item and operation identity, complete authoritative requirements, rooted filesystem access, bounded process supervision, marker-based remote correlation, owned asynchronous extraction, strict configuration domains, and typed correlated partial outcomes. The comparison also established that Codex rollout history is not equivalent to Fox's side-effect-authoritative ledger, Claude's interactive background shell policy does not apply to unattended Autodev, and physical terminal output cannot provide exactly-once delivery across a process crash.
- **User Confirmation**: The user explicitly confirmed all ten proposed correction semantics, including stable logical event identity with idempotent at-least-once reporting instead of an impossible cross-crash exactly-once stdout guarantee.
- **Entries Confirmed**: DEC-046
- **Entries Amended**: CON-005
