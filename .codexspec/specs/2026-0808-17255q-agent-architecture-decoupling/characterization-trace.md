...
| `DV-AOP-001` | blocked-defect | `TestDVAOP001DeduperIsProcessLocalAndClaimsBeforeTerminalWork`, `TestDVAOP001GatewayRejectsMissingAndEmptyMessageIDsBeforeAgentOps` | Shared Gateway rejects missing/empty IDs, but AgentOps' second Deduper accepts empty keys directly, claims before bridge delivery, has no failure/completion rollback, expires only after the exact TTL boundary, and loses all state on restart. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven defect; the second process-local authority can permanently suppress live retries or duplicate post-restart work even though Gateway validation is corrected. |
| `DV-AOP-002` | blocked-defect | `TestDVAOP002ProductionEntryHasNoCoordinatedShutdown`, `TestDVAOP002RunnerReturnsWithoutDrainingAcceptedWork`, `TestDVAOP002AcceptedPermitWaiterIsDroppedOnCancellation` | Production uses background context and fire-and-forget gateway/bridge/runner goroutines; Runner returns before in-flight work and silently drops an accepted global-permit waiter on cancellation. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven defect; two task channels, bridge, HTTP intake, queued work, and in-flight work have no coordinated terminal protocol. |
| `DV-AOP-003` | blocked-defect | `TestDVAOP003PanicHasNoCorrelatedTerminalDelivery`, `TestDVAOP003NonPanicFailuresAttemptOneFallbackWithCurrentContext` | Panic releases capacity and allows a successor but sends no terminal result; ordinary/timeout errors make one fallback attempt, while parent cancellation attempts delivery with an already-cancelled context. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven defect; no typed task outcome authority prevents missing or context-invalid terminal behavior. |
| `DV-AOP-004` | blocked-defect | `TestDVAOP004CompactorDoesNotUseSelectedProviderModel` | AgentOps leaves `CompactionConfig.Model` empty, producing the 128K fallback for a selected registered 200K model. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven defect; engine/provider and compactor model snapshots can diverge. |
| `DV-AOP-005` | blocked-defect | `TestDVAOP005DeliveryFailuresCanEndWithoutDeliveredTerminalOutcome` | Initial notice error is ignored; a 6000-rune final is passed unbounded through the Runner messenger boundary; final failure triggers one fallback whose own failure is discarded, and `Run` returns no terminal outcome. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven defect; completed work can have neither a delivered result nor an observable delivery-failure outcome. |
| `DV-AOP-006` | blocked-defect | `TestDVAOP006LogSearchFollowsSymlinkOutsideLogRoot`, `TestDVAOP006LogSearchResourceBoundsRemainEffective` | `log_search` follows `<logDir>/<service>.log` symlinks and returns outside content; the separate 200-match and one-MiB scanner limits remain effective. | `env GOCACHE=/tmp/fox-go-build-cache go test ./cmd/agentops ./internal/agentops -run 'TestDVAOP|TestDVAOPApproval' -count=1 -timeout=30s` | `3dd89e1` | Proven security defect; lexical service validation does not constrain the final opened target to the configured log root. |
