...
| `DV-CHD-001` | blocked-defect | `TestDVCHD001ReadOnlyChildBashCanMutateInsideAndOutsideWorkspace` | A delegation assessed as low-risk read-only advertises arbitrary Bash and executes direct redirection plus a subprocess that mutates both workspace and outside-root files. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/subagent -run '^TestDVCHD001' -count=1 -timeout=30s` | `03f26ae` | T043 proof; removing write/edit tools does not make unrestricted Bash read-only, and an allowlist is the only current way to remove Bash entirely. |
| `DV-CHD-002` | blocked-defect | `TestDVCHD002ChildModelInvocationAndCompactorUseDifferentSnapshots` | Child model telemetry records the selected `claude-3.5-sonnet`, while its compactor receives no model, resolves the 128K fallback rather than 200K, and compacts a deterministic 120K-token history that the selected-model compactor leaves intact. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/subagent -run '^TestDVCHD002' -count=1 -timeout=30s` | `03f26ae` | T043 proof; invocation and compaction do not share one frozen provider/model snapshot, so thresholds, token headroom, observations, and resulting projection can diverge. |
| `DV-CHD-003` | blocked-defect | `TestDVCHD003PromptAndEffectiveToolSnapshotsDisagree` | The read-only model-visible/callable snapshot contains only Bash and read_file and both child modes exclude delegation/TODO/skill calls, but the composed system prompt instructs the child to use write_file, edit_file, read_todo, and update_todo. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/subagent -run '^TestDVCHD003' -count=1 -timeout=30s` | `03f26ae` | T043 proof; current registry filtering preserves the one-level model surface, but prompt rendering is not derived from the same effective capability snapshot. |
| `DV-CHD-004` | blocked-defect | `TestDVCHD004ChildCancellationKillsShellDescendantsAndPendingApproval` | Parent cancellation kills an active Unix Bash process group with no delayed side effect, and cancelled approval leaves no grant or execution; however, Bash can detach a delayed background process before returning and turn exhaustion leaves it alive to mutate the workspace. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/subagent -run '^TestDVCHD004' -count=1 -timeout=30s` | `03f26ae` | T043 proof; per-call cancellation is sound on Unix, but ChildRun owns no run-scoped process registry that terminal outcomes drain. Codex and Claude Code both retain explicit process/task cleanup authorities. |
| `DV-CHD-005` | blocked-defect | `TestDVCHD005ForkAdapterDropsSelectedAgentBeforeChildInvocation` | The fork adapter passes the processed task, live parent ID, project instruction, selected provider, allowed-tool intersection, and final report, but the accepted selected-agent marker is absent from the child invocation because the adapter discards it. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/app -run '^TestDVCHD005' -count=1 -timeout=30s` | `03f26ae` | T043 proof; Codex applies `agent_role` to child config/source and Claude Code resolves `subagent_type` to an agent definition, confirming this is an invocation-contract loss rather than presentation behavior. |
| `DV-CHD-006` | blocked-defect | `TestDVCHD006ChildWrapperDiscardsCorrelatedOutcomesForEveryFailureClass` | Provider, post-tool provider, persistence, compactor-construction, turn-limit-with-partial-report, and cancellation failures all return an error with a nil child result even though a fresh child session exists and may contain partial messages; session identity and partial report never reach the parent. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/subagent -run '^TestDVCHD006' -count=1 -timeout=30s` | `03f26ae` | T043 proof; the manager collapses engine partial-result/error semantics and pre-engine child-session failures into one uncorrelated nil outcome. Claude Code retains max-turn reason/messages and extracts the latest text-bearing assistant result. |
