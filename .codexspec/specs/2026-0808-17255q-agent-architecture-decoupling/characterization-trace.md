...
| `DV-AUT-001` | blocked-defect | `TestDVAUT001LedgerSaveFailureDoesNotStopIrreversibleWorkflow`, `TestDVAUT001InitialLedgerSaveFailureStopsBeforeWorktreeCreation` | Initial save failure stops before worktree creation, but failures at each later authoritative transition only warn; in-memory execution continues through SDD, issue, PR, done, and cleanup while durable state is unavailable. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT001' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; correction semantics not authorized. |
| `DV-AUT-002` | blocked-defect | `TestDVAUT002UnknownRecordedStagesBypassTheSDDPipeline` | Empty and every known SDD stage map deterministically, but publish, done, renamed, malformed, future, and unknown non-empty values all map past the complete SDD pipeline; a malformed resumed item publishes and becomes done. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT002' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; correction semantics not authorized. |
| `DV-AUT-003` | blocked-defect | `TestDVAUT003BacklogReconciliationUsesMutableTitlesAndRetainsStaleItems` | Title matching safely consumes current duplicate titles and refreshes matching descriptions, but rename creates a second identity, deletion retains stale pending work, reorder retains prior ledger order, and an absent persisted item loses its authoritative description. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT003' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; stable identity and reconciliation remain unresolved. |
| `DV-AUT-004` | blocked-defect | `TestDVAUT004RequirementMaterializationLosesAuthoritativeFormattingAndLength`, `TestDVAUT004BacklogScannerRejectsAnOtherwiseValidLargeDescriptionLine` | Multiline Markdown and fenced code collapse to one line, descriptions truncate at 4,000 runes, and a valid description line over the one-MiB Scanner limit is rejected; Unicode and empty-description title fallback are deterministic. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT004' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; exact authoritative requirement text is not preserved. |
| `DV-AUT-005` | blocked-defect | `TestDVAUT005PersistedFeatureDirEscapesWorktreeForMaterializationAndVerification` | An absolute FeatureDir is lexically reanchored under WorkDir, but traversal writes outside the worktree and both directory and final-file symlinks are followed; read-only artifact verification accepts the escaped targets. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT005' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; correction must cover every artifact read/write consumer. |
| `DV-AUT-006` | blocked-defect | `TestDVAUT006ExecRunnerRetainsUnboundedOutput`, `TestDVAUT006CancellationLeavesDescendantAliveAndStartsLaterGates` | ExecRunner retains a complete unbounded two-MiB stream; caller cancellation kills only the immediate helper and leaves its descendant alive; GateRunner starts all later commands with an already-cancelled context. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/autodev -run '^TestDVAUT006' -count=10 -timeout=90s` | `c8bb3cf` | T044 proof; no stage deadline, output cap, tree ownership, or post-cancel suppression exists. |
