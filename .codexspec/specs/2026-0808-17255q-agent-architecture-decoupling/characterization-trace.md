...
| `DV-BEN-001` | blocked-defect | `TestDVBEN001NoWholeCaseDeadlineAndCancellationDoesNotStopAllValidations` | `RunCase` has no whole-case deadline. A cancelled context fails command validation but does not stop a later context-insensitive file validation, so the evaluation has no single accepted-work terminal boundary. | `env GOCACHE=/tmp/fox-go-build-cache go test ./internal/benchmark ./cmd/bench -run '^TestDVBEN' -count=1 -timeout=30s` | `7561032` | Proven defect; runtime and validation lifetimes are fragmented. |
| `DV-BEN-002` | blocked-defect | `TestDVBEN002FailedResultsDoNotControlProcessStatus` | `cmd/bench` appends failed results, prints and writes them, but contains no success aggregation or non-zero exit path. Only infrastructure errors passed to `log.Fatal` fail the process. | same as above | `7561032` | Proven defect; evaluation failure is reported as process success. |
| `DV-BEN-003` | blocked-defect | `TestDVBEN003RuntimeFidelityHasNoResolvedSpecificationIdentity`, `TestDVBEN003CompositionUsesManuallyMaintainedFidelityClaims` | Fidelity contains only independently maintained string slices and a warning; it omits resolved profile, provider/model, turn budget, tools, memory, and compaction identity and is populated from literals in the composition root. | same as above | `7561032` | Proven defect; reporting can drift from the runtime it claims to describe. |
| `DV-BEN-004` | blocked-defect | `TestDVBEN004FixtureAndValidationCanResolveOutsideOwnedRoots` | Fixture copy dereferences a file symlink outside the fixture; `file_contains` accepts traversal and an outside-target symlink; setup paths contain no failure cleanup for the created temporary workspace. | same as above | `7561032` | Proven security and lifecycle defect; neither source nor result validation is confined to its authority root. |
| `DV-BEN-005` | blocked-defect | `TestDVBEN005InvalidCaseDomainsRemainAccepted`, `TestDVBEN005NonPositiveRepeatCanProduceSuccessfulZeroRunReport` | Negative `max_turns`, empty command, empty path, and empty contains are accepted. Non-positive repeat skips the run loop and reaches a successful empty summary/report path. | same as above | `7561032` | Proven defect; invalid and vacuous evaluation inputs are accepted. |
| `DV-BEN-006` | blocked-defect | `TestDVBEN006CommandValidationIsUnboundedAndImmediateProcessOnly` | Validation uses unbounded `CombinedOutput` and `exec.CommandContext` without process-group setup, bounded stdout/stderr readers, overflow status, or descendant termination/reaping. | same as above | `7561032` | Proven resource and lifecycle defect. |
| `DV-BEN-007` | blocked-defect | `TestDVBEN007ResultOmitsStableExecutionAndDefinitionIdentity` | Result omits repeat index, run ID, case-definition and fixture identity, runtime terminal status, provider protocol, and model. Existing case/session/workspace fields cannot disambiguate reused definitions or fully reproduce a run. | same as above | `7561032` | Proven provenance defect. |
