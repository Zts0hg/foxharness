...
| `DV-AUT-002` | corrected | `TestDVAUT002InvalidRecordedStateFailsBeforeExternalWork`, `TestDVAUT002ResumeIndexDistinguishesRunningAndVerifiedStages`, `TestDVAUT002RunningStageVerifiesBeforeCoreExecution`, `TestDVAUT002RemoteRunningIssueVerifiesBeforeCoreExecution`, `TestLoadLedgerMigratesKnownLegacyStageAndWritesCurrentVersion` | Ledger schema v1 uses a closed typed stage vocabulary and explicit `pending`, `running`, and `verified` states. Versionless known records migrate deterministically and rewrite as v1, while an unexpected legacy `stage_state` fails closed. Unknown stages, future versions, illegal terminal combinations, and verified issue/PR stages without positive bindings return typed `InvalidLedgerStateError` before Git, GitHub, worktree, or core work. A running SDD or remote stage verifies current ground truth first and avoids duplicate execution when already complete; verified stages resume at their successor. | `go test ./internal/autodev -count=1 -timeout=180s`; `go test -race ./internal/autodev -count=1 -timeout=240s`; `go test ./... -count=1 -timeout=300s`; `go vet ./...` | `D-AUT-002` | T104 Green; complete repository gate passed outside the restricted filesystem/listener sandbox after the sandbox run failed only on ambient HOME writes and `httptest` port binding. |
