...
`M03` makes `internal/memory.Store` the only implementation owner for session `working_memory.md`, `PLAN.md`, and `TODO.md` behavior. `internal/session` no longer defines a working-memory type, template, load, append, or replace implementation. To preserve the existing guarantee that a newly returned stored session already has its scratchpad, `FileStore.Create` temporarily delegates only scratchpad initialization to `memory.Store`; the complete runtime mechanism injection at `M12` removes this compatibility edge without making runtime import the concrete memory package. App, benchmark, and AgentOps use `Store.WorkingMemoryPath` directly. Feishu and the old ChildRun assembly retain exactly two deprecated `StoredSession.MemoryPath` calls because their current package boundaries forbid importing the memory mechanism; an automated decreasing ceiling prevents any new use, and the ChildRun (`M15`) and Feishu (`M19`) profile cutovers remove them.
...
`M08` closes the target engine boundary without moving any production profile. `AgentEngine` retains exactly the five confirmed immutable collaborators, and target source imports only its current standard-library primitives plus `internal/schema`; AST gates reject concrete infrastructure, legacy engine contracts, presentation writes, and execution-result fields owned by runtime. The six existing legacy production files form a closed temporary file list, so every new engine production file is checked as target code by default. `Conversation.Prepare` receives immutable invocation input, while `Conversation.RequestChanges` makes mutation ownership explicit: engine submits ordered typed changes but cannot apply, persist, compact, or recover them itself. The future `runtime.ContextController` decides when those requests become authoritative state and prepares the next post-compaction projection at `M11`. The legacy implementation remains in the same Go package only for unmigrated production profiles and is removed at `M25`; its existing allowlisted edges are not target exceptions.
...
`SessionStore` is defined by runtime and implemented by `session.FileStore`; its methods perform create/open/start/finish mechanics only. A run-finish storage failure places the live session in an explicit recovery-required state: new runs and close are rejected, while the same scope may retry the terminal commit. Admission resumes only after that durable commit succeeds, preventing live and persisted lifecycle state from diverging. The lease registry is process-local to the shared harness; composition must share one harness per configured store. Cross-process leases are not part of the existing single-process behavior contract. M11 adds the recoverable conversation/context commit implementation to this sole session authority. M12 adds engine driving, profile-wide capacity, active-run cancellation/join, concrete model/tool/policy/permission snapshots, memory initialization, artifacts, and telemetry; M10 does not claim those later capabilities are already assembled.
